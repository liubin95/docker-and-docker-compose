...
def daily_news():
    ...
    res = [
        {
            "href": f'{root_domain}{item.xpath("./@href").get()}',
...
def daily_weather():
    # beijing dalian
    city_list = [
        {"id": "101010100", "name": "北京市", "content": []},
        {"id": "101070201", "name": "大连市", "content": []},
    ]
    _key = os.getenv("QWEATHER_KEY")
    for item in city_list:
        _res = requests.get(
            f"https://devapi.qweather.com/v7/weather/now?key={_key}&location={item['id']}"
        )
        _json = _res.json()
...
def dail_alcohol():
    ...
    for i in range(1, 15):
        if drink[f"strIngredient{i}"]:
            raw.append(
                {"name": drink[f"strIngredient{i}"], "measure": drink[f"strMeasure{i}"]}
            )
        else:
            break
    res = {
        "name": drink["strDrink"],
        "description": drink["strInstructionsZH-HANS"]
        if drink["strInstructionsZH-HANS"]
        else drink["strInstructions"],
...
@app.get("/weather")
def api_daily():
    return dingding_robot(build_message("markdown", build_markdown(), "天气预报"))
...
def build_markdown_weather():
    env = Environment(
        loader=FileSystemLoader("templates"), autoescape=select_autoescape()
    )
    date = time.strftime("%Y年%m月%d日", time.localtime())
    template = env.get_template("weather.jinja")
    res = template.render(weather_list=daily_weather(), date=date)
...
def dingding_robot(msg):
    ...
    res = requests.post(
        url, headers={"Content-Type": "application/json; charset=utf-8"}, json=msg
    )
